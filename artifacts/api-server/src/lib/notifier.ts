...
async function executePhemexTrade(
  ...
  const DEFAULT_ALLOWED_SIGNAL_TYPES = "FIB50_SWING,BB_REJECTION,DOUBLE_BOTTOM,DOUBLE_TOP,DUMP_RECOVERY,SWING_BREAK";
...
async function checkSymbol(
  ...
  try {
    ...
    // Externally-closed sentinel: persists until the signal naturally transitions
    // to WAIT or flips direction. Price movement alone is NOT sufficient reason
    // to clear — a pumping coin will always move >2% and would re-enter the
    // same losing side immediately. Only a genuine signal flip clears it.
