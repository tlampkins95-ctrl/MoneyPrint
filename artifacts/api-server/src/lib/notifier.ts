...
async function checkSymbol(
  ...
  try {
    ...
    if (transitioned && !cooldownActive && !alreadyInSameDirection) {
      ...
      if (!isFilledTrade) {
        const signalTypeAllowed = levels.signalType === "FIB50_SWING" || levels.signalType === "DOUBLE_TOP" || levels.signalType === "BB_REJECTION";
...
async function checkTrendingSymbol(
  ...
  try {
    ...
    if (transitioned && !cooldownActive && !alreadyInSameDirection) {
      ...
      if (!isFilledTrade) {
        const trendingTypeAllowed = levels.signalType === "FIB50_SWING" || levels.signalType === "DOUBLE_TOP" || levels.signalType === "BB_REJECTION";
