export function ZoneDiagram() {
  ...
  // Fake price path (SVG polyline points)
  ...
  const label = (text: string, price: number, color: string, anchor: "start" | "end" = "end") => (
    <g>
      <text
        x={anchor === "end" ? padL - 4 : padL + chartW + 4}
        y={toY(price) + 4}
        fill={color}
        fontSize="10"
      ...
      <div className="w-full max-w-[680px] space-y-3">
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-3">
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }}>
            {/* Sell zone shaded */}
            <rect
              x={padL} y={toY(sellZoneHigh)}
              width={chartW} height={toY(sellZoneLow) - toY(sellZoneHigh)}
              fill="#ef5350" fillOpacity="0.10"
            />
            {/* Buy zone shaded */}
            <rect
