...
- **Frozen trade snapshots**: When a BUY/SELL fires, the entry/SL/TP/RR/zones/pivot are snapshotted server-side and remain frozen until the trade is invalidated (price hits SL or TP2). This prevents the dashboard "Trade Ticket" values from drifting as new pivot bars roll in. State is persisted to `artifacts/api-server/.runtime/active-trades.json` (gitignored) so it survives server restarts and deploys. See `computeLevelsStable` and `loadActiveTradesFromDisk` in `signals.ts`. Override the file path with `ACTIVE_TRADES_FILE` env var.
