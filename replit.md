...
- **Signal logic**: Single clear BUY / SELL / WAIT signal derived from price levels, NOT from multiple competing indicators
  ...
  - EMA 21/50 for trend bias display (UPTREND/DOWNTREND/RANGING) — informational only, NOT a signal gate. Removed as a gate because EMA21 > EMA50 during a pump is a direct result of the pump itself, which suppresses sell-zone setups we want to trade.
  - Entry gates: RSI exhaustion (≤45 oversold for BUY, ≥55 overbought for SELL) + MACD histogram turning at the zone + EMA200 institutional regime bias.
